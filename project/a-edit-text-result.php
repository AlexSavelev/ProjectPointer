...
if(mb_strlen($name) < 1 || mb_strlen($name) > 200) {
    header('Location: a-edit-text.php?p='.$project_ref.'&t='.$text_id.'&error=Недопустимая%20длина%20заголовка');
    exit;
}
if(mb_strlen($content) > 10000) {
	header('Location: a-edit-text.php?p='.$project_ref.'&t='.$text_id.'&error=Недопустимая%20длина%20содержания');
	exit;
}
...
if($total > 10) {
	header('Location: a-edit-text.php?p='.$project_ref.'&t='.$text_id.'&error=Слишком%20много%20файлов%20для%20одной%20загрузки');
	exit();
}
...
for( $i=0 ; $i < $total ; $i++ ) {
	...
	if($format[count($format)-1] == "jpg" || $format[count($format)-1] == "png" || $format[count($format)-1] == "jpeg") {
		...
		if($img_count >= $limit_imgs) {
			$mysql->close();
			header('Location: a-edit-text.php?p='.$project_ref.'&t='.$text_id.'&error=Достигнут%20лимит%20заргужаемых%20изображений');
			exit();
		}
		// LIMIT SIZE
		if(filesize($_FILES['file']['tmp_name'][$i]) == 0) {
			$mysql->close();
			header('Location: a-edit-text.php?p='.$project_ref.'&t='.$text_id.'&error=Один%20из%20файлов%20слишком%20большой');
			exit();
		}
		...
		if($m_qr_count >= $limit_files) {
			$mysql->close();
			header('Location: a-edit-text.php?p='.$project_ref.'&t='.$text_id.'&error=Достигнут%20лимит%20заргужаемых%20QR-файлов');
			exit();
		}
		// LIMIT SIZE
		if(filesize($_FILES['file']['tmp_name'][$i]) == 0) {
...
for( $i=0 ; $i < $total ; $i++ ) {
	...
	if(!move_uploaded_file($_FILES['file']['tmp_name'][$i], $_SERVER['DOCUMENT_ROOT']."/media/".$project_ref."/".$file_name)) {
		...
		$mysql->close();
		header('Location: a-edit-section.php?p='.$project_ref.'&t='.$text_id.'&error=Не%20удалось%20загрузить%20файл%20');
		exit();
	}
...
$mysql->query("UPDATE $project_ref SET `name` = '$name', `content` = '$content' WHERE `type`='TEXT' AND `ID`='$text_id'");
